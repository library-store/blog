    ...
    <form class="col-sm-9 offset-sm-1" @submit.prevent="onSubmit">
      <div class="col-sm-12">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-10">
            <input type="text" id="title" name="title" v-model="product.title" class="form-control">
          </div>
        </div>
        <div class="form-group row">
          <label for="subtitle" class="col-sm-2 col-form-label">{{ $t('form.subtitle') }}</label>
          <div class="col-sm-10">
            <input type="text" id="subtitle" name="subtitle" v-model="product.subtitle" class="form-control">
          </div>
        </div>
        <div class="form-group row">
          <label for="page_image" class="col-sm-2 col-form-label">{{ $t('form.page_image') }}</label>
          <div class="col-sm-5">
            <input type="text" id="page_image" class="form-control" name="page_image" v-model="product.page_image" placeholder="ex: /uploads/default_avatar.png">
          </div>
          <div class="col-sm-5">
            <img v-if="product.page_image != null && product.page_image != ''" :src="product.page_image" alt="Pig Jian" width="35" height="35">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-10">
            <textarea id="meta_description" name="meta_description" v-model="product.meta_description" class="form-control"></textarea>
          </div>
        </div>
        <div class="form-group row">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-2">
            <div class="togglebutton" style="margin-top: 6px">
              <label>
                <input type="checkbox" name="is_draft" v-model="product.is_draft">
          ...
          <div class="col-sm-2">
            <div class="togglebutton" style="margin-top: 6px">
              <label>
                <input type="checkbox" name="is_original" v-model="product.is_original">
                <span class="toggle"></span>
              </label>
            </div>
          </div>
      ...
      <div class="col-sm-12">
        <div class="form-group row">
          <div class="offset-sm-2 col-sm-10">
            <button type="submit" class="btn btn-info">{{ product.id ? $t('form.edit') : $t('form.create') }}</button>
          </div>
        </div>
      </div>
    </form>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      let tagIDs = []
      let url = 'product' + (this.product.id ? '/' + this.product.id : '')
      let method = this.product.id ? 'patch' : 'post'
      ...
      this.$http[method](url, this.product)
        .then((response) => {
          toastr.success('You ' + this.mode + 'd the product success!')
    ...
    coverUploader(event) {
      ...
      this.$http.post('file/upload', formData)
        .then((response) => {
          ...
          this.product.page_image = response.data.url
        }).catch(({ response }) => {
